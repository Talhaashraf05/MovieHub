function Pop() {
  ...
  `; function formatAMPM(date) { var hours = date.getHours(); var minutes = date.getMinutes(); var ampm = hours >= 12 ? "pm" : "am"; hours = hours % 12; hours = hours ? hours : 12; minutes = minutes < 10 ? "0" + minutes : minutes; var strTime = hours + ":" + minutes + " " + ampm; return strTime; }
    let createPopUp = function () {
        let activeService = content.services[0].name; if (typeof conDivObj === "undefined") { conDivObj = document.createElement("DIV"); conDivObj.setAttribute("id", "freechatpopup"); }
    ...
    `; function serviceSeletor() {
            const service = {}; content.services.map((i) => { service[i.name] = i.content; }); if (activeService === "whatsapp") { document.getElementById("freechat-popup-message").innerHTML = freechatInput; const input = document.getElementById("freechat-popup-input"); const inputSvg = document.getElementById("freechat-popup-input-svg"); input.addEventListener("input", (e) => { input.value.length > 0 ? (inputSvg.style.color = content.color) : (inputSvg.style.color = "#CCCCCC"); }); input.addEventListener("keypress", (e) => { if (e.key === "Enter") { window.open(`https://wa.me/${service.whatsapp}?text=${input.value}!`, "_blank"); } }); inputSvg.addEventListener("click", function () { window.open(`https://wa.me/${service.whatsapp}?text=${input.value}!`, "_blank"); }); } else {
                document.getElementById("freechat-popup-message").innerHTML = freechatNotInput; document.getElementById("freechat-popup-notinput-icon").innerHTML = `
        ...
        content.services.map((item) => { document.getElementById(`fc-icon-${item.name}`).addEventListener("click", function () { activeService = item.name; serviceSeletor(); content.services.map((i) => { document.getElementById(`fc-icon-${i.name}`).classList.remove("active"); }); document.getElementById(`fc-icon-${item.name}`).classList.add("active"); }); });
    }; this.init = function (param) {
        if (typeof param === "object") {
            if ("show" in param) content.show = param.show; if ("logo" in param && param.logo !== null) content.logo = param.logo; if ("logo" in param && param.logo === null) { content.logo = "https://d2r80wdbkwti6l.cloudfront.net/Cz74LbbmsqGrFmpcWlqJaQpx6GJwZyEz.jpg"; }
